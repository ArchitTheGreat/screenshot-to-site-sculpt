    ...
    <div className="min-h-screen bg-background py-8 px-4">
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="font-serif text-fluid-3xl md:text-fluid-4xl font-bold tracking-tight">KryptoGain</h1>
      ...
      <Card className="max-w-4xl mx-auto shadow-xl border-2">
        <CardHeader className="space-y-2">
          <CardTitle className="text-2xl font-serif">Cryptocurrency Tax Calculator</CardTitle>
          <p className="text-sm text-muted-foreground">Upload your transactions and calculate your crypto taxes instantly</p>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <Input
              ...
              className="cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-primary file:text-primary-foreground hover:file:bg-primary/90 transition-all"
            />
            {!csvFile && (
              <p className="text-xs text-muted-foreground">
                Upload a CSV file with columns: type, amount/value, and date
              </p>
            )}
          </div>
          ...
          <div className="space-y-3">
            <Label htmlFor="jurisdiction" className="text-base font-semibold">Tax Jurisdiction</Label>
            <Select value={selectedJurisdiction} onValueChange={handleJurisdictionChange}>
              <SelectTrigger id="jurisdiction" className="h-12 text-base">
                <SelectValue placeholder="Select your tax jurisdiction" />
              </SelectTrigger>
              <SelectContent className="bg-background">
                {jurisdictions.map(j => (
                  <SelectItem key={j.id} value={j.id} className="cursor-pointer">
                    <div className="flex flex-col">
                      <span className="font-semibold">{j.name} - {j.shortTermRate}%</span>
          ...
          {transactions.length > 0 && (
            <Card className="border-2 border-primary/20 shadow-lg animate-fade-in">
              <CardHeader className="pb-3">
                <CardTitle className="text-xl font-serif">Tax Summary Preview</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center p-3 rounded-lg bg-muted/30">
                  <span className="text-sm font-medium text-muted-foreground">Net Profit/Loss:</span>
                  <span className={`text-xl font-bold ${netProfit >= 0 ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'}`}>
                    {formatCurrency(netProfit)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-lg bg-destructive/5">
                  <span className="text-sm font-medium text-muted-foreground">Total Tax Owed:</span>
                  <span className="text-xl font-bold text-destructive">
                    {formatCurrency(totalTax)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-4 rounded-lg bg-primary/5 border border-primary/20">
                  <span className="font-semibold text-foreground">Net After Tax:</span>
          ...
          <div className="space-y-4 pt-6 border-t-2">
            <div className="bg-muted/50 p-4 rounded-lg space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  >
                    I have paid $15 for the tax report
                  </Label>
                </div>
              ...
              <Button
                variant="outline"
                size="lg"
                className="w-full hover-scale border-2"
            ...
            {hasPaid && (
              <Button
                size="lg"
                className="w-full text-lg py-6 shadow-lg hover:shadow-xl transition-all animate-fade-in"
                onClick={generatePDF}
            ...
            {!hasPaid && csvFile && transactions.length > 0 && (
